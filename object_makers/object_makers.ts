import { buttonStyles, IConfirmObject, IDispatchActionConfigObject, IMrkdwnTextObject, IOptionGroupObject, IOptionObject, IPlainTextObject } from "customTypes/custom_types.ts";
...
export const makeOptionObjectsArrayDiffVals = (textVal: Array<[string, string]>) => {
  const options: IOptionObject[] = [];
  textVal.forEach((set) => {
...
export const makeSingleOptionGroup = (label: string, options: IOptionObject[]) => {
...
export const makeOptionGroupsArray = (groups: Array<[string, IOptionObject[]]>) => {
...
export const makeConfirmObject = (title: string, text: string, confirm: string, deny: string, style?: buttonStyles) => {
...
export const makeDispatchConfigObject = (triggers: Array<"on_enter_pressed" | "on_character_entered">) => {
